...
from astronomaly.feature_extraction import pixel_features
...
#     print(f"Found {len(true_anomaly_indices)} ground-truth anomalies.")
    ...
    # If you want to use the full set of shape features, uncomment the next line
    print(f"Extracted {len(features.columns)} shape features from {len(features.index)} objects.")
    from astronomaly.feature_extraction import pixel_features
    pix_feats = pixel_features.PixelStatisticsFeatures(
        output_dir=output_path,
        force_rerun=True
    ).run_on_dataset(dataset)

    features = pd.concat([features, pix_feats], axis=1)
#############################################################
